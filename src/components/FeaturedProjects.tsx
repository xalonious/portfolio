...
function ProjectCard({ project }: { project: Project }) {
  ...
  const CardInner = (
    <motion.article
      variants={item}
      className="group relative mx-auto w-full max-w-[22rem] overflow-hidden rounded-xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-[0_4px_14px_-8px_rgba(0,0,0,0.5)] sm:max-w-none sm:rounded-2xl"
      style={{ transformStyle: "preserve-3d" as any }}
    >
      <div className="relative h-40 sm:h-44 md:h-48 overflow-hidden rounded-t-xl" style={{ perspective: 1000 }}>
        <motion.div
          ref={ref}
      ...
      <div className="p-3 sm:p-4 text-[13px] sm:text-sm leading-relaxed">
        <h3 className="text-base sm:text-lg font-semibold tracking-tight mb-1">{project.title}</h3>
        <p className="text-gray-400 line-clamp-2 sm:line-clamp-3 mb-2">{description}</p>

        <div className="mb-3 flex flex-wrap gap-1.5 sm:gap-2">
          {project.tech.map((t) => (
            <span
              key={t}
              className="rounded-full border border-white/10 bg-white/5 px-2 py-[2px] text-[10px] sm:text-xs text-white/90"
            >
              {t}
            </span>
          ))}
        ...
        <div className="flex gap-2">
          {project.repo && (
            <Button
              asChild
              size="sm"
              className="h-8 text-xs bg-[--primary] text-[--background] hover:bg-[--secondary]"
            >
